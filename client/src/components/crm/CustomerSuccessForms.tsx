...
export function UsageDialog({ open, onOpenChange, organizationId, contacts }: { open: boolean; onOpenChange: (open: boolean) => void; organizationId: number; contacts: any[] }) {
  ...
  const [form, setForm] = useState({ contactId: "none", documentsGeneratedCount: "1", aiRequestsCount: "0", logDate: new Date().toISOString().slice(0, 10) });
  const submit = async (event: FormEvent) => { event.preventDefault(); try { await mutation.mutateAsync({ organizationId, contactId: form.contactId === "none" ? null : Number(form.contactId), documentsGeneratedCount: Number(form.documentsGeneratedCount), aiRequestsCount: Number(form.aiRequestsCount), logDate: form.logDate }); await Promise.all([utils.customerSuccess.invalidate(), utils.analytics.invalidate()]); toast.success("Usage ajouté"); onOpenChange(false); } catch (error) { toast.error(errorMessage(error)); } };
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Ajouter un relevé d’usage</DialogTitle><DialogDescription>Enregistrez les écrits générés et les appels IA pour l’établissement ou un praticien.</DialogDescription></DialogHeader><form onSubmit={submit} className="space-y-4"><Field label="Praticien"><select value={form.contactId} onChange={event => setForm({ ...form, contactId: event.target.value })} className={selectClass}><option value="none">Usage global établissement</option>{contacts.map(contact => <option key={contact.id} value={contact.id}>{contact.fullName}</option>)}</select></Field><div className="grid gap-4 sm:grid-cols-3"><Field label="Écrits générés"><Input required type="number" min="1" value={form.documentsGeneratedCount} onChange={event => setForm({ ...form, documentsGeneratedCount: event.target.value })} className={inputClass} /></Field><Field label="Requêtes IA"><Input required type="number" min="0" value={form.aiRequestsCount} onChange={event => setForm({ ...form, aiRequestsCount: event.target.value })} className={inputClass} /></Field><Field label="Date"><Input required type="date" value={form.logDate} onChange={event => setForm({ ...form, logDate: event.target.value })} className={inputClass} /></Field></div><DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Annuler</Button><Button type="submit" disabled={mutation.isPending}>Ajouter</Button></DialogFooter></form></DialogContent></Dialog>;
}
